...
import districtsJson from "../assets/jsonData/bd-districts.json";
import upazilasJson from "../assets/jsonData/bd-upazilas.json";
import divisionJson from "../assets/jsonData/bd-divisions.json";
import dhakaCityJson from "../assets/jsonData/dhaka-city.json";

const useGetDistrict = (divisionProp) => {
  const [allDivisions, setAllDivisions] = useState(
    divisionJson.divisions || []
  );
  const [allDistricts, setAllDistricts] = useState([]);
  const [needDistricts, setNeedDistricts] = useState([]);
  const [needArea, setNeedArea] = useState([]);
  const [needUnions, setNeedUnions] = useState([]);
  // console.log("division", division);
  const { data: division, isLoading } = useGetDivision();

  useEffect(() => {
    setAllDivisions(divisionJson.divisions);
  }, []);
  ...
  //     await axios
  //       .get(`https://bdapi.vercel.app/api/v.1/district/${divisionProp}`)
  //       .then((res) => {
  //         console.log("res dis", res.data);
  //         // setAllDistricts(district);
  //         // setNeedDistricts(district);
  //       })
  //       .catch((err) => {
  ...
  //   }
  //   fetchDivision();
  //   // setAllDivisions(division);
  //   // setAllDistricts(district);
  //   // setNeedDistricts(district);
  //   // console.log(divisionProp)
  // }, [divisionProp]);

  async function fetchDivisionDistricts(url) {
    return await axios
      .get(url)
      .then((res) => {
  ...
  const getDistricts = async (divisionId) => {
    // let url = `https://bdapi.vercel.app/api/v.1/district/${divisionId}`;
    // const getDistricts = await fetchDivisionDistricts(url);
    const newDistricts = districtsJson.districts
      .filter((item) => item.division_id == divisionId || item)
      ?.map((item) => {
        return {
          label: item.name,
          value: item.name,
    ...
    setNeedDistricts(newDistricts);
    // console.log(data)
  };
  const getAreaByDistricts = async (districtId) => {
    // let url = `https://bdapi.vercel.app/api/v.1/upazilla/${districtId}`;
    // const getUpazilla = await fetchDivisionDistricts(url);
    const others = {
      id: "other",
      name: "other",
    };
    let newAreas;
    if (districtId === "1") {
      newAreas = dhakaCityJson.dhaka?.map((item) => {
        return {
          label: item.name,
          value: item.name,
          ...item,
      ...
      newAreas = upazilasJson.upazilas
    ...
    setNeedArea([...newAreas]);
    // setneedArea([...getUpazilla, others]);
    // console.log(data)
  };
  const getUnionsByUpazilla = async (unionId) => {
    let url = `https://bdapi.vercel.app/api/v.1/union/${unionId}`;
    const getUnions = await fetchDivisionDistricts(url);
    const others = {
      id: "other",
      name: "other",
    };
    setNeedUnions([...getUnions, others]);
  };

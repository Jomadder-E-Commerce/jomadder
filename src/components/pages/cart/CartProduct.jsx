                    ...
                    <div className="pt-4 border-b">
                        <div className="flex items-center justify-between">
                          ...
                          <div className='flex gap-4'>
                                ...
                                <RiDeleteBin6Line disable={isLoading} onClick={() => handleDeleted(product?.id)} className='text-xl text-red-700 bg-white cursor-pointer  block ' />

                          </div>
                                <div>
                                    <Link href={`/product-details/${product?.id}`} className="text-sm font-semibold">Order ID: #{product.id}</Link>
                                    <Link href={`/product-details/${product?.id}`} className="text-sm pr-4">{product.productTitle.length > 30 ? product.productTitle.slice(0, 30) + "..." : product.productTitle}</Link>
                                </div>
                            </div>
                            {/* <RiDeleteBin6Line disable={isLoading} onClick={() => handleDeleted(product?.id)} className='text-xl  text-red-700 bg-white cursor-pointer hidden sm:block' /> */}
                        </div>
                    </div>
                        ...
                        <div className='' key={sku._id}>
                            <div className='flex border-b flex-col  py-2   justify-between gap-3'>
                                <div className='items-center gap-3 flex'>
                                    <Image unoptimized src={sku.image} alt="SKU" width={40} height={40} className='object-cover ' />
                                    <p className="text-sm">{sku.sku?.split(";").join(", ")}</p>
                                </div>
                                <div className='flex justify-between items-end'>
                                <p className='text-sm'>{sku.quantity} * ৳{parseFloat(sku.price).toFixed(2)}</p>
                                <p className=' block'>৳{(sku.quantity * parseFloat(sku.price)).toFixed(2)}</p>
                                </div>
